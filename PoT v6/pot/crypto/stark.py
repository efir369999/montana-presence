...
from pot.constants import VDF_CHECKPOINT_INTERVAL
...
try:
    import pot_stark
    _pot_stark = pot_stark
    _STARK_RUST_AVAILABLE = True
    logger.info(f"pot_stark Rust module available (v{pot_stark.version()})")
except ImportError:
    logger.warning(
        "pot_stark Rust module not available. "
...
def generate_stark_proof(
    ...
    if _STARK_RUST_AVAILABLE:
        ...
        return _pot_stark.generate_stark_proof(
            initial_state.data,
            final_output.data,
            iterations,
            checkpoint_bytes
...
def verify_stark_proof(
    ...
    if _STARK_RUST_AVAILABLE:
        # Use native Rust implementation
        return _pot_stark.verify_stark_proof(
            initial_state.data,
            final_output.data,
            iterations,
            proof
...
def get_stark_info() -> dict:
    ...
    if _STARK_RUST_AVAILABLE:
        info["native_version"] = _pot_stark.version()
