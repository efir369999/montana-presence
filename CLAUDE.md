...
| **PERMISSION SYNC** | `PresenceEngine.swift` | Синхронизация разрешений: старт → автоотключение без разрешения, переходы → авто вкл/выкл (кроме manual-only: activity/wifi/autostart) |
...
6. **Manual-only датчики** (activity, wifi, autostart): НЕ автовключаются при permission grant, только вручную пользователем
