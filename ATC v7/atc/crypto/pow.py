"""
ATC Protocol v7 Personal Proof of Work
...
try:
    import atc_randomx
    _atc_randomx = atc_randomx
    _RANDOMX_AVAILABLE = True
    logger.info(f"atc_randomx available (v{atc_randomx.version()})")
except ImportError:
    logger.warning(
        "atc_randomx not available. "
...
def compute_randomx(data: bytes) -> bytes:
    ...
    """
    if _RANDOMX_AVAILABLE and _atc_randomx.is_initialized():
        return bytes(_atc_randomx.randomx_hash(data))
    else:
...
def init_randomx(epoch_key: bytes, full_mem: bool = False):
    ...
    if _RANDOMX_AVAILABLE:
        _atc_randomx.init_vm(epoch_key, full_mem)
...
def get_pow_info() -> dict:
    ...
    return {
        ...
        "stage2_initialized": _RANDOMX_AVAILABLE and _atc_randomx.is_initialized() if _RANDOMX_AVAILABLE else False,
...
def print_difficulty_table():
    ...
    print("\nATC Personal PoW Difficulty Table")
    print("=" * 60)
