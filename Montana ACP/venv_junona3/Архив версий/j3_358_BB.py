...
# Генерация ключа шифрования (в реальном приложении сохраните его в безопасном месте)
...
def get_session_key():
    """
    Запрашивает у пользователя ключ сессии и возвращает его в зашифрованном виде.
    """
    ...
    # Шифруем ключ сессии
    encrypted_session_key = cipher.encrypt(session_key.encode())
    # Очищаем исходный ключ из памяти
    session_key = None
    return encrypted_session_key


def get_api_key_from_bitwarden(encrypted_session_key, item_name):
    """
    Получает элемент (например, API-ключ) из Bitwarden по имени элемента, используя зашифрованный ключ сессии.
    """
    # Расшифровываем ключ сессии
    session_key = cipher.decrypt(encrypted_session_key).decode()
    cmd = ["bw", "get", "item", item_name, "--session", session_key]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    stdout, stderr = process.communicate()
    # Очищаем session_key из памяти
    session_key = None
    if process.returncode != 0:
        log_event(f"Ошибка при получении {item_name}: {stderr}")
        raise Exception(f"Не удалось получить {item_name} из Bitwarden")
    item = json.loads(stdout)
    # Шифруем полученный ключ
    encrypted_key = cipher.encrypt(item['notes'].encode())
    return encrypted_key


def get_decrypted_key(encrypted_key):
    """
    Расшифровывает зашифрованный ключ и возвращает его в виде строки.
    """
...
try:
    encrypted_session_key = get_session_key()
    log_event("Получен зашифрованный session key")
except Exception as e:
    log_event(f"Произошла ошибка при получении session key: {e}")
    exit(1)

# Получение зашифрованных API-ключей
encrypted_BYBIT_API_KEY = get_api_key_from_bitwarden(encrypted_session_key, "api_key_peskov_profit")
encrypted_BYBIT_API_SECRET = get_api_key_from_bitwarden(encrypted_session_key, "RSA Private Key for API")

# Расшифровка ключей только на момент инициализации клиента
BYBIT_API_KEY = get_decrypted_key(encrypted_BYBIT_API_KEY)
BYBIT_API_SECRET = get_decrypted_key(encrypted_BYBIT_API_SECRET)
...
encrypted_session_key = None  # Очистка зашифрованного session key
...
TRADING_CONFIG = {
    ...
    'ENABLE_LOGGING': True,
    'IMPULSE_ENTRY_PERCENT': 3,  # Процент от доступного баланса для импульсных сделок
...
def test_encryption():
    """
    Проверяет корректность шифрования и расшифровки данных с использованием cipher.
    """
    try:
        test_data = "test_string"  # Тестовая строка
        encrypted_data = cipher.encrypt(test_data.encode())  # Шифруем строку
        decrypted_data = cipher.decrypt(encrypted_data).decode()  # Расшифровываем результат
        if test_data == decrypted_data:
            log_event("✅ Тест шифрования: успех")  # Успех, если строки совпадают
        else:
            log_event("❌ Тест шифрования: ошибка")  # Ошибка, если строки различаются
    except Exception as e:
        log_event(f"⚠️ Ошибка в тесте шифрования: {e}")  # Логируем ошибку, если что-то пошло не так
