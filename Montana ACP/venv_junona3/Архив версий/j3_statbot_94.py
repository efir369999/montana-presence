...
TELEGRAM_TOKEN = keyring.get_password("telegram_token_stat", "TG_bot")
...
binance_client = Client(BINANCE_API_KEY, BINANCE_SECRET_KEY, {"timeout": 30})
...
async def inline_refresh_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading_message = await context.bot.send_message(chat_id=chat_id, text="⏳ Loading data, please wait...")
...
async def inline_generic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_data == "main_menu":
        await query.edit_message_text(
            text="Вы берите действие в Меню",
            reply_markup=get_main_menu_buttons()
        )
        return
...
async def check_and_send_orders(context: ContextTypes.DEFAULT_TYPE):
    try:
        # 1. Обновляем файл статистики (binance_stat_j2.csv)
        save_stat()
            ...
            with open(STAT_FILE, "r") as file:
                ...
                for row in reader:
                    if row["Stat Type"] == "Trade":
                        ...
                        try:
                            # Пытаемся преобразовать Trade ID в целое число для корректного сравнения
                        ...
                        if last_sent_time is None:
                            ...
                            if trade_time > last_sent_time:
                                ...
                                try:
                                    if int(trade_id) > int(last_sent_id):
                                        include = True
                                except Exception as e:
                        ...
                        if include:
                            ...
                            if new_last_time is None or trade_time > new_last_time:
                                ...
                                try:
                                    if int(trade_id) > int(new_last_id):
                                        new_last_id = trade_id
                                except Exception as e:
...
async def send_updates_to_user(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    try:
        # Обновляем и сохраняем статистику перед отправкой обновлений пользователю
        save_stat()
        ...
        for pos in open_positions:
            ...
            message += (f"• {symbol}: объем. {volume:.3f} ({position_size:.2f}$)\n"
        ...
        total_balance_with_pnl = total_balance + unrealized_pnl  # верно
        

        # Расчет средней прибыли за один день в долларах и процентах
        total_days = (datetime.now(timezone.utc) - initial_deposit_date).days
        average_daily_profit_dollars = total_balance_with_pnl / total_days
        ...
        # Создаем инлайн кнопки "Wallet Details" и "Trade Details"
        keyboard = [
            [InlineKeyboardButton("📎 Прямая ссылка на статистику", url='https://www.binance.com/en/copy-trading/lead-details/4147851193928698881')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)

        # Отправляем сообщение с обновленными кнопками
        ...
        if os.path.exists(STAT_FILE):
            with open(STAT_FILE, "rb") as file:
                await context.bot.send_document(chat_id=user_id, document=file, caption="Файл статистики Binance")
        else:
...
def get_futures_account_snapshot(limit=7):
    """
    Запрашивает снимок фьючерсного аккаунта.
    Параметр limit задаёт количество возвращаемых снимков (максимум 7).
    """
    endpoint = "/sapi/v1/accountSnapshot"
    timestamp = int(time.time() * 1000)
    params = {
        "type": "FUTURES",  # Фьючерсный аккаунт
        "limit": limit,     # Количество дней (снимков)
        "timestamp": timestamp
    }
    ...
    response = requests.get(f"https://api.binance.com{endpoint}", params=params, headers=headers)
...
def save_stat():
    ...
    start_time = get_last_saved_timestamp() - 60 * 1000  # с небольшим запасом
    ...
    while start_time < end_time:
        try:
            ...
            # Добавляем проверку, чтобы не запрашивать данные в будущем
            ...
            trade_data = binance_client.futures_account_trades(startTime=start_time, endTime=interval_end)
            if trade_data:
                for trade in trade_data:
                    ...
                    if trade_id not in existing_ids:
                        ...
                        entry = {
                            "Trade ID": trade_id,
                            "Time": datetime.fromtimestamp(trade.get('time', 0) / 1000, timezone.utc).strftime('%Y-%m-%d %H:%M:%S'),
                            "Symbol": trade.get('symbol', 'N/A'),
                            "Side": "Buy" if trade.get('side', '').lower() == "buy" else "Sell",
                            "Price": float(trade.get('price', 0)),
                            "Quantity": float(trade.get('qty', 0)),
                            "Total": float(trade.get('quoteQty', 0)),
                            "Fee": fee,
                            "Fee Asset": trade.get('commissionAsset', 'N/A'),
                            "Role": "Maker" if trade.get('isMaker', False) else "Taker",
                            "Realized Profit": realized_pnl,
                            "Net Realized Profit": net_realized_pnl,
                            "Cumulative Net Realized Profit": None,
                            "Stat Type": "Trade",
                            "Transfer Amount": "",
                            # Дополнительные поля (snapshot) оставляем пустыми
                            "walletBalance": "",
                            "marginBalance": "",
                            "markPrice": "",
                            "calculatedPnL": ""
            ...
            # Запрос истории доходов (income)
            income_data = binance_client.futures_income_history(startTime=start_time, endTime=interval_end)
            if income_data:
                for inc in income_data:
                    income_type = inc.get('incomeType', '')
                    if income_type not in ["FUNDING_FEE", "TRANSFER"]:
                        continue
                    income_id = str(inc.get('tranId', ''))
                    if income_id and income_id not in existing_ids:
                        income_val = float(inc.get('income', 0))
                        ...
                        entry = {
                            "Trade ID": income_id,
                            "Time": datetime.fromtimestamp(inc.get('time', 0) / 1000, timezone.utc).strftime('%Y-%m-%d %H:%M:%S'),
        ...
        # Обработка данных account snapshot за последние 7 дней
        snapshot_data = get_futures_account_snapshot(7)
        snapshots = snapshot_data.get("snapshotVos", [])
        for snapshot in snapshots:
            # Используем время из API (timestamp в мс) и форматируем через utcfromtimestamp
            update_time = datetime.fromtimestamp(snapshot["updateTime"] / 1000, timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
            for asset in snapshot["data"]["assets"]:
                for position in snapshot["data"]["position"]:
                    ...
                    if side == "LONG":
                        calculated_pnl = (mark_price - entry_price) * abs(position_size)
                    else:
                        calculated_pnl = (entry_price - mark_price) * abs(position_size)
                    margin_balance = wallet_balance + calculated_pnl

                    # Генерация уникального идентификатора для snapshot
                    snapshot_id = f"snapshot_{update_time}_{asset['asset']}_{entry_price}_{position_size}"
                    # Если такой ID уже есть, пропускаем запись
                    if snapshot_id in existing_ids:
                        continue

                    entry = {
                        "Trade ID": snapshot_id,  # Используем сгенерированный ID вместо пустой строки
                        "Time": update_time,                 
                        "Symbol": asset["asset"],            
                        "Side": side,
                        "Price": entry_price,                
                        ...
                        "calculatedPnL": calculated_pnl
                    }
                    new_entries.append(entry)
                    existing_ids.add(snapshot_id)

    # Сортируем все записи по времени (используя поле "Time")
    try:
        new_entries.sort(key=get_sort_time)
    except Exception as e:
        logging.error(f"Ошибка сортировки записей: {e}")

    # Обновляем накопленный итог для записей сделок и income (snapshot оставляем без изменений)
...
if __name__ == '__main__':
    ...
    application.add_handler(CallbackQueryHandler(inline_generic, pattern="^(account|wallet|partners_program|create_bot|connect_bybit|support)$"))
