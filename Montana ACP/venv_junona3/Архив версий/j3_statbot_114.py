...
async def send_updates_to_user(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    try:
        ...
        # Средняя дневная прибыль рассчитывается от реализованной прибыли из файла статистики
        average_daily_profit_dollars = cumulative_realized_profit / total_days if total_days > 0 else 0
        average_daily_profit_percent = (average_daily_profit_dollars / cumulative_realized_profit * 100) if cumulative_realized_profit != 0 else 0
        ...
        message += "*\n💸 Детали прибыли:*\n"
        message += f"• Средняя за 1 день: {average_daily_profit_dollars:,.2f}$ ({average_daily_profit_percent:,.2f}%)\n"
        message += f"• Средняя за 30 дней: {average_monthly_profit_dollars:,.2f}$ ({average_monthly_profit_percent:,.2f}%)\n\n"
...
def generate_cumulative_profit_chart(stat_file: str | None = None) -> BytesIO | None:
    ...
    try:
        ...
        ax.plot(x, y, color="#00a000", linewidth=3)
        ...
        span_days = max(1, int((x.iloc[-1] - x.iloc[0]).days))
        ...
        pad = (ymax - ymin) * 0.05 or max(1.0, abs(ymax) * 0.02) if ymax != 0 else 1.0
        ...
        d0, d1 = x.iloc[0].strftime("%d.%m.%Y"), x.iloc[-1].strftime("%d.%m.%Y")
        days = (x.iloc[-1].date() - x.iloc[0].date()).days + 1
        ...
        if not df_last_5m.empty:
            ...
            minmax_text = f"{days} дней (с {d0})  |  Макс за 5 мес: {max_5m:,.2f} USDT  |  Мин за 5 мес: {min_5m:,.2f} USDT"
        else:
            minmax_text = f"{days} дней (с {d0})"
        
        fig.suptitle(
            f"Реализованная прибыль Юноны  |  Дата: {d1}  |  Текущая прибыль: {last_profit:,.2f} USDT\n{minmax_text}",
            color="white",
            fontsize=12,
            y=0.98,
            ha="center",
...
def generate_balance_chart(stat_file: str | None = None, current_balance: float | None = None) -> BytesIO | None:
    ...
    try:
        ...
        ymin, ymax = float(np.nanmin(y)), float(np.nanmax(y))
        pad = (ymax - ymin) * 0.05 or max(1.0, ymax * 0.02)
        ax.set_ylim(ymin - pad, ymax + pad)
        ax2.set_ylim(ax.get_ylim())
        ...
        fig.suptitle(
            f"Баланс Юноны  |  Дата: {d1}  |  Текущий баланс: {last_bal:,.2f} USDT\n{minmax_text}",
            color="white",
            fontsize=12,
            y=0.98,
            ha="center",
...
if __name__ == '__main__':
    ...
    # Ежедневное сохранение баланса кошелька в 23:55 по Московскому времени
    job_queue.run_daily(save_daily_balance_snapshot, time=dt.time(hour=23, minute=55, tzinfo=moscow_time))
