...
def get_next_market_change_date(current_date):
    if TEST_MODE:
        return TEST_MARKET_TYPE, TEST_NEXT_CHANGE
    if current_date.tzinfo is None:
        current_date = current_date.replace(tzinfo=timezone.utc)
    date_day = current_date.date()
    if not market_periods:
...
def load_market_data(market_type):
    ...
    try:
        if MARKET_DATA_FILE.exists():
            ...
            # Установка глобальных значений только для включенных индикаторов
            if len(df) >= 2:
                # RSI, если включен
                if (market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_RSI']) or (market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_RSI']):
                ...
                # Stochastic RSI, если включен
                if (market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_STOCHRSI']) or (market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_STOCHRSI']):
                ...
                # Williams %R overbought, если включен
                if (market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_WILLIAMS_OVERBOUGHT']) or (market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_WILLIAMS_OVERBOUGHT']):
                ...
                # Williams %R oversold, если включен
                if (market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_WILLIAMS_OVERSOLD']) or (market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_WILLIAMS_OVERSOLD']):
...
def update_market_data_on_candle_close(symbol, timeframe, current_time, limit=242, end_time=None):
    ...
    # RSI (универсально, но сохраняем только если включен)
    if len(closes_np) >= rsi_period and ((current_market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_RSI']) or (current_market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_RSI'])):
    ...
    # Stochastic RSI (универсально, но сохраняем только если включен)
    if len(closes_np) >= stochrsi_rsi_period and ((current_market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_STOCHRSI']) or (current_market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_STOCHRSI'])):
    ...
    # Williams %R overbought (универсально, но сохраняем только если включен)
    if len(df_market) >= williams_overbought_period and ((current_market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_WILLIAMS_OVERBOUGHT']) or (current_market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_WILLIAMS_OVERBOUGHT'])):
    ...
    # Williams %R oversold (универсально, но сохраняем только если включен)
    if len(df_market) >= williams_oversold_period and ((current_market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_WILLIAMS_OVERSOLD']) or (current_market_type == 'bear' and TRADING_CONFIG['ENABLE_BEAR_WILLIAMS_OVERSOLD'])):
...
def check_signals(current_price):
    ...
    with trades_lock:
        ...
        # Универсальная проверка наличия данных для индикаторов (NaN если выключены или нет данных)
        if np.isnan(current_rsi) or np.isnan(current_sma_rsi) or np.isnan(current_stoch_k) or np.isnan(current_stoch_d) or np.isnan(current_williams_r_overbought) or np.isnan(current_williams_r_oversold):
...
def run():
    ...
    # --- Новые переменные для тестового режима (раскомментируйте и задайте значения для тестирования) ---
    global TEST_MODE, TEST_MARKET_TYPE, TEST_NEXT_CHANGE
    TEST_MODE = True  # Установите True / False для активации тестового режима
    TEST_MARKET_TYPE = 'bear'  # Задайте тип рынка вручную ('bull' или 'bear')
    TEST_NEXT_CHANGE = datetime(2025, 10, 1, tzinfo=timezone.utc)  # Задайте дату и время смены рынка вручную
