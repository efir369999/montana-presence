...
# Cycle 2 bear
# BACKTEST_START_DATE = datetime(2017, 12, 11, tzinfo=pytz.UTC)  # datetime(2023, 1, 1, tzinfo=pytz.UTC) или None для использования всего диапазона
...
def calculate_market_periods(df):
    ...
    for idx in range(1, len(halvings)):
        ...
        # Находим понедельник после халвинга (начало недели)
        ...
        # Находим индекс понедельника в df
        try:
            ...
            continue
        # Находим индексы 74 и 78 недель назад
        i_74 = i_halving - 74
        i_78 = i_halving - 78
        if i_74 < 0 or i_78 < 0:
        ...
        # Находим пик: +152 недели от дна (по датам, даже за пределами df)
        peak_date = bottom_date + timedelta(weeks=152)
        # Смена на bear: +1 неделя после пика
        change_to_bear = peak_date + timedelta(weeks=1)
        # Bear start: change_to_bear
    ...
    # Добавление периода для цикла 1 (bear) начиная с 25 ноября 2013 года до начала цикла 2 bull
    if market_periods:
        market_periods.insert(0, {'cycle': 1, 'type': 'bear', 'start': datetime(2013, 11, 25, tzinfo=pytz.UTC), 'change': market_periods[0]['start']})
    # Вывод в логи
    for period in market_periods:
        ...
        logger.info(f"{type_en} {period['cycle']} ({start_str} - {end_str}) смена {change_str}")
...
def close_all_trades_sim(reason, exit_time, current_price):
    ...
    """
    global active_trades, current_balance, current_trade_type, pending_action
    with trades_lock:
        if not active_trades:
        ...
        # Логика разворота сделок после закрытия только для обычных сигналов (не для смены рынка)
        if reason != "market_type_change":
            ...
            # Для смены типа рынка: immediate открытие предпочтительной позиции (даже без сделок)
            current_market_type = get_market_type(exit_time)
            if current_market_type == 'bull':
    ...
    try:
        ...
        for i in range(start_idx, len(closes)):
            ...
            if last_market_type != current_market_type:
                logger.info(f"Смена типа рынка на {current_time} (с {last_market_type} на {current_market_type})")
            ...
            if current_market_type is None:
                # Обновление last_market_type в конце, даже при пропуске
                last_market_type = current_market_type
                continue # Пропускаем выключенный тип рынка без логирования
            if current_market_type == 'bull':
            ...
            # Определяем, являются ли индикаторы нужными для текущего рынка
            ...
            # Пропуск только если nan в нужных индикаторов
            ...
            # Проверка сигнала по индексу страха в начале свечи и выполнение действия на open (только для включенного рынка, после pending_action, с пропуском на свече обнаружения смены)
            fear_greed_value = get_fear_greed_value(current_time)
            ...
            # Обновление last_market_type в конце цикла
            last_market_type = current_market_type
        ...
        if closed_trades.empty:
            ...
            # Инициализация для типов рынков
            ...
            # Расчёт минимальной и максимальной просадки в процентах и соответствующих ID сделок для общего
            ...
            # Расчёт просадок по типам рынков (только для включенных рынков)
