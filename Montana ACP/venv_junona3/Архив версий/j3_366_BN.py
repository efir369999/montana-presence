...
TRADING_CONFIG = {
    ...
    'ENABLE_LOGGING': True,
    'IMPULSE_ENTRY_PERCENT': 5,  # Процент от доступного баланса для импульсных сделок
...
def display_position():
    ...
    with trades_lock:
        ...
        try:
            ...
            # Проверка наличия ключа 'leverage'
            if 'leverage' in position:
                leverage = float(position['leverage'])
            else:
                leverage = None
                log_event("⚠️ Ключ 'leverage' отсутствует в данных позиции")
            ...
            log_event("------------------ ПОЗИЦИЯ -------------------|")
            log_event("----------------------------------------------|")
            log_event(f"{'🟢' if direction == 'LONG' else '🔴'} {direction} | Плечо: {leverage if leverage is not None else 'N/A'}x")
