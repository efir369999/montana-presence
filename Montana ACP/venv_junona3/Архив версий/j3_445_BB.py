...
def calculate_market_periods(df):
    ...
    log_event("📅 Начало расчёта периодов рынка на основе халвингов")
    market_periods = []
    for idx in range(1, len(halvings)):
        ...
        log_event(f"📥 Загрузка данных для цикла {cycle}: с {start_time.strftime('%Y-%m-%d')} по {end_time.strftime('%Y-%m-%d')}")
        df_cycle = load_historical_data(symbol, '1w', start_time=start_time, end_time=end_time)
        log_event(f"📊 Загружено {len(df_cycle)} свечей для цикла {cycle}")
        ...
        try:
            ...
            log_event(f"📍 Индекс халвинга в данных: {i_halving}")
        except KeyError:
            log_event(f"⚠️ Понедельник после халвинга {monday_after.strftime('%Y-%m-%d')} не найден в данных, пропуск цикла {cycle}")
            continue
        i_74 = i_halving - 74
        i_78 = i_halving - 78
        if i_74 < 0 or i_78 < 0:
        ...
        log_event(f"📉 Низкие цены: 74 недели назад - {low_74:.2f}, 78 недель назад - {low_78:.2f}")
        if low_74 < low_78:
            bottom_i = i_74
        else:
            bottom_i = i_78
        bottom_date = df_cycle.index[bottom_i]
        log_event(f"📉 Дата дна цикла {cycle}: {bottom_date.strftime('%Y-%m-%d')}")
        ...
        log_event(f"📈 Дата пика: {peak_date.strftime('%Y-%m-%d')}, смена на медвежий: {change_to_bear.strftime('%Y-%m-%d')}")
    ...
    for period in market_periods:
        ...
        change_str = period['change'].strftime('%Y-%m-%d')
        log_event(f"📊 {type_en.upper()} цикл {period['cycle']}: {start_str} - {end_str}, смена {change_str}")
    log_event("📅 Завершение расчёта периодов рынка")
...
def get_market_type(date):
    ...
    log_event(f"🔍 Определение типа рынка для даты: {date.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    for period in market_periods:
        log_event(f"Проверка периода: цикл {period['cycle']}, тип {period['type']}, start {period['start'].strftime('%Y-%m-%d')}, change {period['change'].strftime('%Y-%m-%d')}")
...
def get_next_market_change_date(current_date):
    ...
    log_event(f"🔍 Поиск следующей даты смены рынка для: {current_date.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    for period in market_periods:
        log_event(f"Проверка периода: цикл {period['cycle']}, тип {period['type']}, start {period['start'].strftime('%Y-%m-%d')}, change {period['change'].strftime('%Y-%m-%d')}")
        if period['start'] <= current_date < period['change']:
            log_event(f"✅ Найден текущий период: тип {period['type']}, смена {period['change'].strftime('%Y-%m-%d %H:%M:%S %Z')}")
...
def run():
    ...
    if current_time.tzinfo is None:
        current_time = current_time.replace(tzinfo=timezone.utc)
    log_event("📅 Определение текущего типа рынка и даты смены при запуске")
    market_type = get_market_type(current_time)
    current_market, next_change_date = get_next_market_change_date(current_time)
    if market_type is not None:
        log_event(f"📈 Текущий тип рынка: {market_type}")
        if next_change_date is not None:
            log_event(f"🔄 Дата следующей смены рынка: {next_change_date.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        else:
