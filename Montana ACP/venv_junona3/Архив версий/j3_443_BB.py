...
START_DATE = datetime(2015, 1, 19, tzinfo=timezone.utc) # Начальная дата в UTC
CYCLE_LENGTH = 204 # Длина цикла (в единицах CYCLE_LENGTH_UNIT)
BULL_LENGTH = 152 # Длина бычьего рынка (в единицах CYCLE_LENGTH_UNIT)
CYCLE_LENGTH_UNIT = 'weeks'  # Единица измерения: 'days', 'minutes', 'hours' и т.д.
...
def get_market_type(date):
    ...
    completed_units = math.floor(delta_units)
    cycle_position = (completed_units % CYCLE_LENGTH) + 1
...
def get_next_market_change_date(current_date):
    ...
    if delta_units < 0:
        return None, None
    completed_units = math.floor(delta_units)
    cycle_number = completed_units // CYCLE_LENGTH
    cycle_position_0 = completed_units % CYCLE_LENGTH
    cycle_position = cycle_position_0 + 1
    current_market = 'bull' if cycle_position <= BULL_LENGTH else 'bear'
    if current_market == 'bull':
        next_change_weeks = cycle_number * CYCLE_LENGTH + BULL_LENGTH
        if CYCLE_LENGTH_UNIT == 'minutes':
            next_change_delta = timedelta(minutes=next_change_weeks)
        elif CYCLE_LENGTH_UNIT == 'hours':
            next_change_delta = timedelta(hours=next_change_weeks)
        elif CYCLE_LENGTH_UNIT == 'days':
            next_change_delta = timedelta(days=next_change_weeks)
        elif CYCLE_LENGTH_UNIT == 'weeks':
            next_change_delta = timedelta(days=next_change_weeks * 7)
        next_change_date = START_DATE + next_change_delta
    else:
        next_change_weeks = (cycle_number + 1) * CYCLE_LENGTH
        if CYCLE_LENGTH_UNIT == 'minutes':
            next_change_delta = timedelta(minutes=next_change_weeks)
        elif CYCLE_LENGTH_UNIT == 'hours':
            next_change_delta = timedelta(hours=next_change_weeks)
...
def sync_active_trades():
    ...
    log_event(f"📅 Текущий тип рынка: {current_market_type} | Следующая смена: {next_change_date.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    if exchange_trades:
