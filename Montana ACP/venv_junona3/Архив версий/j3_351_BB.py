...
if BYBIT_API_KEY is None or BYBIT_API_SECRET is None:
    raise Exception("Не установлены переменные окружения: BYBIT_API_KEY, BYBIT_API_SECRET.")
...
MIN_DELTA_LIQUIDATION_LONG = 22.0  # Минимальная дельта для лонг-позиций
MIN_DELTA_LIQUIDATION_SHORT = 22.0 # Минимальная дельта для шорт-позиций
...
#     try:
#         instruments_info = client.get_instruments_info(category="linear", symbol=symbol)
#         if instruments_info['retCode'] != 0:
#             raise ValueError(f"Ошибка API: {instruments_info['retMsg']}")
#         instrument = instruments_info['result']['list'][0]
#         qty_step = float(instrument['lotSizeFilter']['qtyStep'])
#         precision = int(round(-math.log10(qty_step), 0))
#         return precision
#     except Exception as e:
#         log_event(f"Ошибка при получении информации о символе: {e}")
#         return 5  # Возвращаем значение по умолчанию, если не удалось получить информацию
