...
# Путь к CSV-файлу
CSV_FILE = Path(f"trades_binance.csv")
...
def manage_liquidation_price():
    ...
    for attempt in range(3):
        try:
            positions = client.futures_position_information(symbol=symbol)
            active_positions = [pos for pos in positions if pos['positionAmt'] != '' and float(pos['positionAmt']) != 0]
            if not active_positions:
                log_event("⚪ Нет позиций для управления рисками")
                return
            position = active_positions[0]
            
            # Проверка и преобразование positionAmt
            try:
                size = abs(float(position['positionAmt']))
            except ValueError:
                log_event("⚠️ Ошибка: positionAmt не является числом")
                return
            
            direction = 'LONG' if float(position['positionAmt']) > 0 else 'SHORT'
            
            # Проверка и преобразование liquidationPrice
            try:
                liquidation_price = float(position['liquidationPrice'])
            except ValueError:
                log_event("⚠️ Ошибка: liquidationPrice не является числом")
                return
            
            current_price = get_current_price_with_retries(client, symbol)
            if current_price is None:
                log_event("⚠️ Не удалось получить текущую цену")
                return
    ...
    if TRADING_CONFIG['ENABLE_LOGGING']:
        if CSV_FILE.exists():
            try:
                ...
                if df_trades.empty:
                    log_event("⚠️ CSV файл пуст, создается новый DataFrame")
                    df_trades = pd.DataFrame(columns=headers)
                    df_trades = df_trades.astype(dtypes)
                else:
                ...
                # Безопасное приведение Position_Size к числу
                ...
                log_event(f"⚠️ Ошибка при загрузке CSV: {e}")
                df_trades = pd.DataFrame(columns=headers)
                df_trades = df_trades.astype(dtypes)
        else:
            df_trades = pd.DataFrame(columns=headers)
...
def display_position():
    """Отображает информацию о текущей позиции на Binance Futures, вычисляя PnL вручную."""
    with trades_lock:
        if not active_trades:
            log_event("⚪ Нет активных позиций")
            return
        ...
        try:
            # Получение данных о позиции с Binance Futures
            position_response = client.futures_position_information(symbol=symbol)
            active_positions = [pos for pos in position_response if pos['positionAmt'] != '' and float(pos['positionAmt']) != 0]
            if not active_positions:
                log_event("⚪ Нет активных позиций")
                return
            position = active_positions[0]

            # Проверка и преобразование размера позиции
            try:
            ...
            # Проверка и преобразование цены входа
            try:
                entry_price = float(position['entryPrice'])
            except ValueError:
                log_event("⚠️ Ошибка: entryPrice не является числом")
                entry_price = None

            # Проверка и преобразование цены ликвидации
            try:
                liquidation_price = float(position['liquidationPrice'])
            except ValueError:
                log_event("⚠️ Ошибка: liquidationPrice не является числом")
            ...
            # Вычисление размера позиции в USDT
            size_usd = size * current_price

            # Вычисление нереализованного PnL вручную
            ...
            # Определение индикатора PnL
            pnl_indicator = "🟢" if unrealised_pnl >= 0 else "🔴"

            # Вычисление дельты между текущей ценой и ценой ликвидации
            delta_percent = None
            ...
            # Получение плеча из конфигурации или данных позиции
            current_time = datetime.now()
            market_type = get_market_type(current_time)
            if market_type == 'bull':
                trade_type = 'BULL_LONG' if direction == 'LONG' else 'BULL_SHORT'
                ...
                trade_type = None
            leverage = TRADING_CONFIG.get(trade_type, {}).get('LEVERAGE', 'Неизвестно')

            # Вывод информации в заданном формате
            log_event("----------------------------------------------|")
            log_event("-------------| BINANCE ПОЗИЦИЯ  |-------------|")
            log_event("----------------------------------------------|")
            ...
            log_event(f"💥 Ликвидация: {'--' if liquidation_price is None or liquidation_price <= 0 else f'{liquidation_price:,.2f}'} USDT | Дельта: {'--' if delta_percent is None else f'{delta_percent:.2f}%'}")
