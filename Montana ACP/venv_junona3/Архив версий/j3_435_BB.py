...
# BULL_STOCHRSI_D_PERIOD = 28  # Smooth D
# BULL_STOCHRSI_RSI_PERIOD = 14  # RSI Length
# BULL_STOCHRSI_STOCH_PERIOD = 20  # Stochastic Length
...
BULL_STOCHRSI_RSI_PERIOD = 14  # RSI Length
BULL_STOCHRSI_STOCH_PERIOD = 20  # Stochastic Length
BULL_WILLIAMS_OVERBOUGHT_PERIOD = 99
BULL_WILLIAMS_OVERBOUGHT_LEVEL = -0.01
BULL_WILLIAMS_OVERSOLD_PERIOD = 99
BULL_WILLIAMS_OVERSOLD_LEVEL = -99.9
BULL_FEAR_GREED_LOW = 9
...
# BEAR_WILLIAMS_OVERBOUGHT_LEVEL = -10.0
# BEAR_WILLIAMS_OVERSOLD_PERIOD = 10
# BEAR_WILLIAMS_OVERSOLD_LEVEL = -84.60
...
BEAR_STOCHRSI_RSI_PERIOD = 10  # RSI Length
BEAR_STOCHRSI_STOCH_PERIOD = 16  # Stochastic Length
BEAR_WILLIAMS_OVERBOUGHT_PERIOD = 99
BEAR_WILLIAMS_OVERBOUGHT_LEVEL = -0.01
BEAR_WILLIAMS_OVERSOLD_PERIOD = 99
BEAR_WILLIAMS_OVERSOLD_LEVEL = -99.90
BEAR_FEAR_GREED_HIGH = 52
...
global_df_market = None  # Глобальный DataFrame для данных рынка, ограничивается 400 строками
...
def get_market_data_file(market_type):
    if market_type == 'bull':
        return Path(f"market_data_bull_{script_name}_{GLOBAL_TIMEFRAME}_{BULL_RSI_PERIOD}_{BULL_SMA_RSI_PERIOD}_{BULL_STOCHRSI_K_PERIOD}_{BULL_WILLIAMS_OVERBOUGHT_PERIOD}_{BULL_WILLIAMS_OVERSOLD_PERIOD}.csv")
    elif market_type == 'bear':
        return Path(f"market_data_bear_{script_name}_{GLOBAL_TIMEFRAME}_{BEAR_RSI_PERIOD}_{BEAR_SMA_RSI_PERIOD}_{BEAR_STOCHRSI_K_PERIOD}_{BEAR_WILLIAMS_OVERBOUGHT_PERIOD}_{BEAR_WILLIAMS_OVERSOLD_PERIOD}.csv")
    else:
...
# CYCLE_LENGTH = 1428 # Длина цикла (в единицах CYCLE_LENGTH_UNIT)
# BULL_LENGTH = 1064 # Длина бычьего рынка (в единицах CYCLE_LENGTH_UNIT)
# CYCLE_LENGTH_UNIT = 'days'  # Единица измерения: 'days', 'minutes', 'hours' и т.д.

# Константы для определения циклов рынка
START_DATE = datetime(2015, 1, 12, tzinfo=timezone.utc) # Начальная дата в UTC
CYCLE_LENGTH = 5 # Длина цикла (в единицах CYCLE_LENGTH_UNIT)
BULL_LENGTH = 3 # Длина бычьего рынка (в единицах CYCLE_LENGTH_UNIT)
CYCLE_LENGTH_UNIT = 'minutes'  # Единица измерения: 'days', 'minutes', 'hours' и т.д.



def get_delta_units(delta, unit):
    if unit == 'minutes':
...
def get_market_type(date):
    ...
    delta_units = get_delta_units(delta, CYCLE_LENGTH_UNIT)
    if delta_units < 0:
        return None # Дата до начальной точки, обработка не требуется
    cycle_position = delta_units % CYCLE_LENGTH
    if cycle_position < BULL_LENGTH:
        return 'bull'
    else:
        return 'bear'
...
def get_next_market_change_date(current_date):
    ...
    if delta_units < 0:
        return None, None
    cycle_number = delta_units // CYCLE_LENGTH
    cycle_position = delta_units % CYCLE_LENGTH
    current_market = 'bull' if cycle_position < BULL_LENGTH else 'bear'
    if current_market == 'bull':
        remaining_in_bull = BULL_LENGTH - cycle_position
        if CYCLE_LENGTH_UNIT == 'minutes':
            next_change_delta = timedelta(minutes=remaining_in_bull)
        elif CYCLE_LENGTH_UNIT == 'hours':
            next_change_delta = timedelta(hours=remaining_in_bull)
        elif CYCLE_LENGTH_UNIT == 'days':
            next_change_delta = timedelta(days=remaining_in_bull)
        elif CYCLE_LENGTH_UNIT == 'weeks':
            next_change_delta = timedelta(days=remaining_in_bull * 7)
        next_change_date = current_date + next_change_delta
    else:
        remaining_in_bear = CYCLE_LENGTH - cycle_position
        if CYCLE_LENGTH_UNIT == 'minutes':
            next_change_delta = timedelta(minutes=remaining_in_bear)
        elif CYCLE_LENGTH_UNIT == 'hours':
            next_change_delta = timedelta(hours=remaining_in_bear)
        elif CYCLE_LENGTH_UNIT == 'days':
            next_change_delta = timedelta(days=remaining_in_bear)
        elif CYCLE_LENGTH_UNIT == 'weeks':
            next_change_delta = timedelta(days=remaining_in_bear * 7)
        next_change_date = current_date + next_change_delta
    if next_change_date.tzinfo is None:
        next_change_date = next_change_date.replace(tzinfo=timezone.utc)
    return current_market, next_change_date
...
def load_market_data(market_type):
    ...
    MARKET_DATA_FILE = get_market_data_file(market_type)
    try:
        if global_df_market is not None:
            df = global_df_market.copy()  # Используем копию глобального DF в памяти
        elif MARKET_DATA_FILE.exists():
            df = pd.read_csv(
                MARKET_DATA_FILE,
                parse_dates=['time'],
        ...
        if len(df) >= 2:
            ...
            current_williams_r_overbought = previous_williams_r_overbought = np.nan
            current_williams_r_oversold = previous_williams_r_oversold = np.nan
            log_event("Файл MARKET_DATA пустой, загружаю данные для расчета индикаторов. ")
        return df
    except Exception as e:
...
def save_market_data(df, market_type):
    MARKET_DATA_FILE = get_market_data_file(market_type)
    try:
        df = df.tail(20).copy()  # Сохраняем только последние 20 строк
...
def update_market_data_on_candle_close(symbol, timeframe, current_time, limit=400, end_time=None):
    ...
    delay = 5
    candles = None
    if global_df_market is None or len(global_df_market) < MAX_INDICATOR_PERIOD:
        # Полная загрузка при инициализации
        ...
        log_event("📥 Полная загрузка 400 свечей при инициализации")
    else:
        # Актуализация: запрос только 5 новых свечей
        last_time = global_df_market['time'].max()
        start_time_ms = int((last_time + timedelta(microseconds=1)).timestamp() * 1000)
        limit = 5  # Ограничение для актуализации
        log_event("📥 Актуализация: загрузка последних 5 свечей")
    for attempt in range(max_retries):
        try:
    ...
    if global_df_market is None:
        df_market = new_df
    else:
        # Удаляем старые записи в диапазоне запрошенных свечей
        min_time = new_df['time'].min()
        max_time = new_df['time'].max()
        df_market = global_df_market[(global_df_market['time'] < min_time) | (global_df_market['time'] > max_time)]
    ...
    df_market = df_market.tail(400)  # Ограничение до 400 последних строк
    ...
    if len(df_market) >= williams_overbought_period:
        df_market['Williams_R_Overbought'] = talib.WILLR(highs_np, lows_np, closes_np, timeperiod=williams_overbought_period).astype(np.float64)
    else:
        df_market['Williams_R_Overbought'] = np.full(len(df_market), np.nan, dtype=np.float64)
    # Williams %R oversold
    if len(df_market) >= williams_oversold_period:
        df_market['Williams_R_Oversold'] = talib.WILLR(highs_np, lows_np, closes_np, timeperiod=williams_oversold_period).astype(np.float64)
    else:
        df_market['Williams_R_Oversold'] = np.full(len(df_market), np.nan, dtype=np.float64)
    del closes_np, highs_np, lows_np # Явное освобождение памяти
    global_df_market = df_market  # Обновляем глобальный DF
    ...
    with trades_lock:
        ...
        if current_market_type == 'bull' and TRADING_CONFIG['ENABLE_BULL_MARKET']:
            if not active_trades:
                ...
                if TRADING_CONFIG['ENABLE_BULL_FEAR_GREED'] and fear_greed_value is not None and fear_greed_value <= 26:
        ...
        # Логика для медвежьего рынка
            ...
            if not active_trades:
                ...
                if TRADING_CONFIG['ENABLE_BEAR_FEAR_GREED'] and fear_greed_value is not None and fear_greed_value >= 52:
...
def run():
    ...
    global last_fear_greed_update, last_market_type  # Добавлена инициализация last_market_type
    global global_df_market  # Добавлена глобальная переменная для market data
    # Настройка логирования
    setup_logging()
    # Инициализация файлов с данными свечей только для актуального типа рынка
    current_time = get_server_time()
    if current_time.tzinfo is None:
        current_time = current_time.replace(tzinfo=timezone.utc)
    market_type = get_market_type(current_time)
    if market_type is not None:
        initialize_market_data_file(market_type)
    else:
        log_event("⚠️ Тип рынка не определён при запуске, пропуск инициализации файла market_data")
    # Инициализация даты последнего обновления fear_greed
    last_fear_greed_update = current_time.date()
    # Загрузка и сохранение данных индекса страха и жадности (один раз при запуске)
...
###################################################################################################
    ...
    # Инициализация глобального DF после первой полной загрузки
    global_df_market = load_market_data(market_type)
    # Первоначальный расчет всех индикаторов из файла
    market_type = get_market_type(current_time)
    if not global_df_market.empty and len(global_df_market) >= MAX_INDICATOR_PERIOD:
    ...
    last_market_type = market_type # Инициализация last_market_type
    while True:
        try:
            current_time = get_server_time()
            if current_time.tzinfo is None:
                current_time = current_time.replace(tzinfo=timezone.utc)
            market_type = get_market_type(current_time)
            # Проверка смены типа рынка вне check_signals
            if last_market_type != market_type:
                log_event(f"🔄 Смена типа рынка с {last_market_type} на {market_type}. Закрытие всех сделок.")
                close_all_trades(f"market_type_change_to_{market_type}", force_close=True)
                last_market_type = market_type
                # Инициализация файла market_data для нового типа рынка
                initialize_market_data_file(market_type)
                # Загрузка свечей в новый файл
                update_market_data_on_candle_close(symbol, GLOBAL_TIMEFRAME, current_time)
                # Обновление глобальных индикаторов
                global_df_market = load_market_data(market_type)
                # Открытие новой сделки после смены рынка, если нет активных (независимо от индикаторов)
                if not active_trades:
                    position_value = (get_available_balance() * TRADING_CONFIG['IMPULSE_ENTRY_PERCENT']) / 100
                    if market_type == 'bull':
            ...
            if next_global_update_time is None or current_time >= next_global_update_time:
                current_price = get_current_price_with_retries(client, symbol)
                # Проверка смены суток и обновление данных fear_greed только при смене
                if current_time.date() != last_fear_greed_update:
                    fear_greed_data = fetch_fear_greed_data()
                    if not fear_greed_data:
                        log_event("Не удалось получить данные индекса страха и жадности")
                    last_fear_greed_update = current_time.date()
            ...
            if next_rsi_update_time is None or current_time >= next_rsi_update_time:
                current_price = get_current_price_with_retries(client, symbol)
                # Обновление данных в market_data при завершении свечи GLOBAL_TIMEFRAME
                update_market_data_on_candle_close(symbol, GLOBAL_TIMEFRAME, current_time)
                # Глобальный DF уже обновлён в update..., индикаторы пересчитаны
                if not global_df_market.empty and len(global_df_market) >= MAX_INDICATOR_PERIOD:
                    # Глобальные значения уже установлены в load_market_data (но поскольку используем global_df_market, они актуальны)
                    pass
                else:
                    log_event("⚠️ Недостаточно данных в market_data для индикаторов")
                ...
                # Добавленное логирование текущего типа рынка и следующей смены
                current_market, next_change_date = get_next_market_change_date(current_time)
                if current_market and next_change_date:
                    log_event(f"🔄 Тип рынка: {current_market}, смена: {next_change_date.strftime('%Y-%m-%d %H:%M:%S %Z')}")
                else:
