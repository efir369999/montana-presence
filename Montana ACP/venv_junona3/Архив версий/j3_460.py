...
MIN_DELTA_LIQUIDATION_LONG = 3.0 # Минимальная дельта для лонг-позиций
...
def display_position():
    with trades_lock:
        ...
        try:
            ...
            realized_pnl_percent = (realized_pnl / initial_margin * 100) if initial_margin > 0 else 0.0
            unrealized_pnl_percent = (unrealized_pnl / initial_margin * 100) if initial_margin > 0 else 0.0
            ...
            log_event(f"{realized_indicator} Реализованная прибыль: {realized_pnl:,.2f}$ ({realized_pnl_percent:.2f}%)")
            log_event(f"{unrealized_indicator} Не реализованная прибыль: {unrealized_pnl:,.2f}$ ({unrealized_pnl_percent:.2f}%)")
