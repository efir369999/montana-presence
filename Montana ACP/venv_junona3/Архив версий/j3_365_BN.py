...
def open_trade(trade_type, entry_price, position_value=None, trailing_status=None):
    """Открывает новую сделку на束 2 на Binance Futures с изолированной маржой."""
    ...
    with trades_lock:
        ...
        try:
            ...
            log_event(f"✅ Плечо установлено на {leverage}x, тип маржи: ISOLATED")
        except BinanceAPIException as e:
            log_event(f"⚠️ Ошибка установки плеча или типа маржи: {e}")
...
def adjust_leverage_after_partial_close(direction, min_delta):
    """Корректирует плечо после частичного закрытия позиции для поддержания дельты."""
    global client, symbol, MIN_DELTA_LIQUIDATION_LONG, MIN_DELTA_LIQUIDATION_SHORT
    ...
    while delta_percent < min_delta and current_leverage > min_leverage:
        ...
        try:
            ...
            log_event(f"✅ Плечо уменьшено до {new_leverage}x")
        except BinanceAPIException as e:
            log_event(f"⚠️ Ошибка уменьшения плеча: {e}")
            break
        time.sleep(5)
        ...
        if delta_percent >= min_delta:
            ...
            log_event(f"⚠️ Плечо достигло минимального значения {min_leverage}x, дельта: {delta_percent:.2f}%")
            break
...
# Используй только официальные библиотеки Binance
