...
async def send_updates_to_user(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    try:
        ...
        # Отправляем график баланса ПЕРВЫМ (если есть snapshot'ы)
        try:
            chart = generate_balance_chart()
            if chart:
                await context.bot.send_photo(chat_id=user_id, photo=chart)
        except Exception as e:
            logging.error(f"Ошибка при отправке графика пользователю {user_id}: {e}", exc_info=True)
...
def generate_balance_chart(stat_file: str | None = None) -> BytesIO | None:
    ...
    try:
        ...
        ax2.set_ylabel("Balance (USDT)", color="white")
