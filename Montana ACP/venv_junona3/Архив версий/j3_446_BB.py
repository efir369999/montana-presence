...
def calculate_market_periods(df):
    ...
    log_event("📅 Начало расчёта периодов рынка на основе последнего халвинга")
    market_periods = []
...
def sync_active_trades():
    ...
    if current_market_type is None:
        ...
        return
    # current_market, next_change_date = get_next_market_change_date(current_time)  # Не вызываем повторно, используем глобальные
    log_event(f"📅 Текущий тип рынка: {current_market_type} | Следующая смена: {next_market_change.strftime('%Y-%m-%d %H:%M:%S %Z') if next_market_change else 'не определена'}")
    if exchange_trades:
...
def run():
    ...
    global current_market_type, next_market_change  # Глобальные для кэширования
    setup_logging()
