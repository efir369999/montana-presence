...
AUTHORIZED_IDS = [6148271304, 5249406291, 8552053404, 8059342690]
...
def group_trades(entries):
    """
    Универсальная функция группировки сделок типа "Trade" по времени до минут,
    символу и направлению. Возвращает список сгруппированных записей.
    Income-события возвращаются без изменений.
    """
    # Разделяем сделки и income-события
    trade_entries = []
    income_entries = []
    
    for entry in entries:
        if entry.get("Stat Type") == "Trade" and entry.get("Side") and entry.get("Side") != "":
    ...
    # Объединяем сгруппированные сделки с income-событиями
    all_entries = grouped_entries + income_entries
...
async def send_updates_to_user(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    try:
        ...
        total_balance_with_pnl = total_balance + unrealized_pnl  # Учитываем суммарный unrealized PnL
...
FIELDNAMES = [
    ...
    "Cumulative Net Realized Profit", "Stat Type"
]
...
def save_stat():
                ...
                for trade in trade_data:
                    ...
                    if trade_id not in existing_ids:
                        ...
                        entry = {
                            ...
                            "Fee": round(fee, 2),
                            "Realized Profit": round(realized_pnl, 2),
                            "Net Realized Profit": round(net_realized_pnl, 2),
            ...
            if income_data:
                for inc in income_data:
                    ...
                    if income_id and income_id not in existing_ids:
                        ...
                        # Применяем округление для числовых значений (2 знака после запятой)
                        entry = {
                            "Trade ID": income_id,
                            ...
                            "Fee": "",
                            "Realized Profit": "",
                            "Net Realized Profit": round(net_profit_value, 2),
                            "Cumulative Net Realized Profit": None,
    ...
    for entry in new_entries:
        if entry["Time"] and entry["Stat Type"] != "Snapshot":
            ...
            # Округляем Cumulative Net Realized Profit до 2 знаков после запятой
...
if __name__ == '__main__':
    ...
    application.add_handler(CallbackQueryHandler(inline_generic, pattern="^trades$"))
