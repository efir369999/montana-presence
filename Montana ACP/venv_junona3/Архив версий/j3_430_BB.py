...
def get_next_market_change_date(current_date):
    ...
    if current_market == 'bull':
        ...
        next_change_delta = timedelta(**{CYCLE_LENGTH_UNIT: remaining_in_bull})
        next_change_date = current_date + next_change_delta
    else:
        remaining_in_bear = CYCLE_LENGTH - cycle_position
        next_change_delta = timedelta(**{CYCLE_LENGTH_UNIT: remaining_in_bear})
        next_change_date = current_date + next_change_delta
...
def load_market_data(market_type):
    ...
    try:
        if MARKET_DATA_FILE.exists():
            ...
            # Ограничение размера DataFrame до последних 200 строк
            df = df.tail(200)
...
def save_market_data(df, market_type):
    ...
    try:
        # Ограничение размера DataFrame до последних 200 строк
        df = df.tail(200).copy()
        # Преобразование числовых столбцов в float32 для оптимизации
        numeric_cols = ['open', 'high', 'low', 'close', 'RSI', 'RSI-based MA', 'StochRSI_K', 'StochRSI_D', 'Williams_R_Overbought', 'Williams_R_Oversold']
        for col in numeric_cols:
            if col in df.columns:
...
def update_market_data_on_candle_close(symbol, timeframe, current_time, limit=200, end_time=None):
    ...
    df_market['RSI'] = rsi.astype(np.float32)
    if len(rsi) >= sma_rsi_period:
        rsi_for_sma = rsi.astype(np.float64) # Преобразование в float64 для SMA
        sma_rsi = talib.SMA(rsi_for_sma, timeperiod=sma_rsi_period)
    else:
        sma_rsi = np.full(len(rsi), np.nan, dtype=np.float64)
    df_market['RSI-based MA'] = sma_rsi.astype(np.float32)
    # Stochastic RSI
    if len(closes_np) >= stochrsi_rsi_period:
        ...
        df_market['StochRSI_K'] = fastd.astype(np.float32)
        if len(fastd) >= stochrsi_d_period:
            fastd_for_sma = fastd.astype(np.float64) # Преобразование в float64 для SMA
            df_market['StochRSI_D'] = talib.SMA(fastd_for_sma, timeperiod=stochrsi_d_period).astype(np.float32)
        else:
            df_market['StochRSI_D'] = np.full(len(fastd), np.nan, dtype=np.float32)
    else:
        df_market['StochRSI_K'] = np.full(len(closes_np), np.nan, dtype=np.float32)
        df_market['StochRSI_D'] = np.full(len(closes_np), np.nan, dtype=np.float32)
    # Williams %R overbought
    if len(df_market) >= williams_overbought_period:
        df_market['Williams_R_Overbought'] = talib.WILLR(highs_np, lows_np, closes_np, timeperiod=williams_overbought_period).astype(np.float32)
    else:
        df_market['Williams_R_Overbought'] = np.full(len(df_market), np.nan, dtype=np.float32)
    # Williams %R oversold
    if len(df_market) >= williams_oversold_period:
        df_market['Williams_R_Oversold'] = talib.WILLR(highs_np, lows_np, closes_np, timeperiod=williams_oversold_period).astype(np.float32)
    else:
        df_market['Williams_R_Oversold'] = np.full(len(df_market), np.nan, dtype=np.float32)
...
def check_signals(current_price):
    ...
    with trades_lock:
        ...
        if 'last_market_type' not in globals():
            ...
            # Открытие новой сделки после смены рынка, если нет активных
...
def run():
    ...
    global last_fear_greed_update # Новая глобальная переменная для отслеживания даты последнего обновления fear_greed
